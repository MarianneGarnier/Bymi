...
public class PlacedOrderResource {
    ...
    private final PlacedOrderSearchRepository placedOrderSearchRepository;

    public PlacedOrderResource(PlacedOrderRepository placedOrderRepository, PlacedOrderSearchRepository placedOrderSearchRepository) {
        this.placedOrderRepository = placedOrderRepository;
        this.placedOrderSearchRepository = placedOrderSearchRepository;
    }
    ...
    public ResponseEntity<PlacedOrder> createPlacedOrder(@RequestBody PlacedOrder placedOrder) throws URISyntaxException {
        ...
        placedOrderSearchRepository.save(result);
    ...
    public ResponseEntity<Void> deletePlacedOrder(@PathVariable Long id) {
        ...
        placedOrderSearchRepository.deleteById(id);
        return ResponseEntity.noContent().headers(HeaderUtil.createEntityDeletionAlert(applicationName, false, ENTITY_NAME, id.toString())).build();
    }

    /**
     * {@code SEARCH  /_search/placed-orders?query=:query} : search for the placedOrder corresponding
     * to the query.
     *
     * @param query the query of the placedOrder search.
     * @return the result of the search.
     */
    @GetMapping("/_search/placed-orders")
    public List<PlacedOrder> searchPlacedOrders(@RequestParam String query) {
        log.debug("REST request to search PlacedOrders for query {}", query);
        return StreamSupport
            .stream(placedOrderSearchRepository.search(queryStringQuery(query)).spliterator(), false)
